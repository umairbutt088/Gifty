...
function titleAndBodyFor(
  ...
  if (event === "new_order") {
    return {
      title: "New order",
      body: `You have a new order for "${giftTitle}".`,
    };
  }
  ...
  return {
    title: "Order update",
    body: status
      ? `Your order for "${giftTitle}" is now ${status}.`
      : `Your order for "${giftTitle}" was updated.`,
  };
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { title, body } = titleAndBodyFor(
      event,
      giftTitle,
      status ?? row.status,
    );

    // channelId is required for reliable Android display with FCM V1.
    // Must match the channel created in registerForPushNotificationsAsync.
    ...
    console.log("[send-order-push-notification]", {
      event,
      orderId,
      recipientUserId,
    ...
    return jsonResponse({ ok: true, sent: messages.length, expoResult });
  } catch (error) {
